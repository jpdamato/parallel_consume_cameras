...
actions_list =[ "0.Preparing room",    
 "1. Cleanning with duster",
 "2. Cleaning surfaces with cloth",
 "3. Cleaning Bathroom (out of scene)",
 "4. Replace trash bags",
 "5. Replace bed sheets",
 "6. Mopping" ]
...
def compute_best_crop(persons,frames,img_small, frame_index):
    ...
    return crop, best_frame, crop_img
...
def detect_persons(model, tracker, img_small, findex):
    ...
    crop, best_frame, crop_img = compute_best_crop(persons, frames, img_small, findex)

    return persons, crop_img , best_frame
...
def parallel_combine_videos(video_files,freq_ollama):
    ...
    while True:
        # Make sure we have all frames
        if all(frames[cam_id] is not None for cam_id in CAMERA_IDS):
            # Resize frames for display (e.g., 320x240 each)
            resized = [cv2.resize(frames[cam_id], (640, 480)) for cam_id in CAMERA_IDS]
            ...
            persons, crop, best_frame = detect_persons(model, tracker, combined, findex)
            ...
            cv2.imshow("YOLO + SORT Tracking", combined)
           # print (f"update {findex}")
            ...
            #annotated_frame = results[0].plot()
            if crop is not None and (crop.shape[0] * crop.shape[1]) > 0:
                cv2.imshow("crop", crop)
                accumulated_detections += 1
            
            output_frames[0] = frames[best_frame].copy()
...
if __name__ == "__main__":
    ...
    print ("-------------------------------------------------------")
    print("Version of parallel combined Cameras . 30set2025 vA")
    print ("-------------------------------------------------------")
    if sys.platform == 'win32':
        print("Running on Windows")
        path = "E:/Resources/Novathena/multicam_2/"
        video_files = [path +"cam1_trimmed.mp4", path +"cam2_trimmed.mp4", path +"cam3_trimmed.mp4", path +"cam4_trimmed.mp4"]
    else:
    ...
    t = threading.Thread(target=run_app, args=(int(args.port),  ))
    t.daemon = True
    t.start()
    


    parallel_combine_videos(video_files, int(args.freq_ollama))
